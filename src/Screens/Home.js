    ...
    <View style={styles.containerTransaction}>
      <View style={styles.profileContainer}>
        {data.category === 'Top Up' ? null : (
          <Image
            source={data.photo === null ? defaultProfile : {uri: profilImg}}
      ...
      {data.type === 'in' ? (
        <Text
          ...
          }>{`+Rp${data.amount.toLocaleString('id-ID')}`}</Text>
      ) : (
        <Text
          style={
            styles.textTransactionNumberOutcome
          }>{`-Rp${data.amount.toLocaleString('id-ID')}`}</Text>
      )}
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.containerHeader}>
        <View style={styles.profileContainer}>
          <Pressable onPress={() => navigation.navigate('Profile')}>
            <Image
              source={photo === null ? defaultProfile : {uri: profilImg}}
              style={styles.profileImg}
            />
          </Pressable>
          <View style={styles.textHelloContainer}>
            ...
            {last_name === null ? (
              ...
              <Text
                style={styles.textName}>{`${first_name} ${last_name}`}</Text>
            )}
          </View>
        </View>
        <Icon name="bell" size={30} color={color.dark} />
      </View>
      <View style={styles.containerBalance}>
        <Text style={styles.textBalanceLabel}>Balance</Text>
        <Text style={styles.textBalanceNumber}>{`Rp${balance.toLocaleString(
          'id-ID',
        )}`}</Text>
        <Text style={styles.textPhoneNumber}>
          {phone === null ? 'Phone Number Not Set' : phone}
        </Text>
      </View>
      <View style={styles.buttonTransferContainer}>
        {phone === null ? (
          <Button
            title="Transfer"
            icon={<Icon name="arrow-up" size={20} color="#608DE2" />}
        ...
        <Button
          title="Top Up"
          icon={<Icon name="plus" size={20} color="#608DE2" />}
          onPress={() =>
            Alert.alert(
              'Coming Soon',
              'Top Up Feature in Process',
              [
                {
                  text: 'Ok',
                },
